...
def list_file(CapFiles):
    ...
    for file in files:
        if file in dbfiles:
            ...
            sql_exec(
                    'insert into pcap (file,pcapnum,size) values ("' + file + '",' + str(
                            pcapnum) + ',"' + filesize + '");')
...
# 获取包来源端口
def get_port_src(file):
    dstport = []
    pcap = rdpcap(UPLOAD_FOLDER + file)
    for packet in pcap:
        if TCP in packet:
            dstport.append(packet.getlayer('TCP').sport)
...
def get_mail(file):
    mailpkts = []
    result = ""
    cap = pyshark.FileCapture(UPLOAD_FOLDER + file)
    for packet in cap:
        if  packet.highest_layer == 'SMTP' \
                or packet.highest_layer == 'POP3' \
                or packet.highest_layer == 'IMAP4':
            mailpkts.append(packet)
    for packet in mailpkts:
        if packet[packet.transport_layer].flags == '0x0018':
            result = result + '''<div class="ui vertical segment"><p>'''

            old_stdout = sys.stdout
            sys.stdout = mystdout = StringIO()

            packet[packet.highest_layer].pretty_print()
            sys.stdout = old_stdout

            result = result + mystdout.getvalue().replace(' ', '&nbsp;').replace('\n', '<br/>').replace('\\r\\n', '')
            result = result + '''</p></div>'''
    if result == "":
        result = '''<div class="ui vertical segment"><p>No Mail Packets!</p></div>'''
    return result
